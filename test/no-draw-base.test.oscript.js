...
describe('Check predication AA', function () {
	...
	it('Create predication', async () => {
		...
		expect(vars1.draw_asset).to.not.exist;
	...
	it('Alice issue tokens', async () => {
		...
		const oldBalance = await this.alice.getBalanceOf(this.predication_address);
		const oldReserveBalance = oldBalance.base.total;
		...
		const target_reserve = Math.ceil(Math.sqrt(yes_amount ** 2 + no_amount ** 2));
		...
		this.reserve = response.balances.base;
	...
	it('Alice issue tokens (not enough reserve)', async () => {
		...
		const new_reserve = Math.ceil(Math.sqrt((this.supply_yes + yes_amount) ** 2 + (this.supply_no + no_amount) ** 2));
		...
		expect(response.response.error).to.equal(`expected reserve amount: ${Math.abs(reserve_needed + 1e4)}`)
		this.reserve = response.balances.base;
	});
	...
	it('Alice redeem yes tokens', async () => {
		...
		const new_reserve = Math.ceil(Math.sqrt(this.supply_yes ** 2 + this.supply_no ** 2));
	...
	it('Bob issue tokens', async () => {
		const yes_amount = 9432250;
		const no_amount = 6642350;
		...
		const target_reserve = Math.ceil(Math.sqrt(this.supply_yes ** 2 + this.supply_no ** 2));
	...
	it('Bob commit result', async () => {
		...
		expect(vars.result).to.be.equal('no');
		this.reserve = response.balances.base;
	});
	...
	it('Alice claim profit', async () => {
		...
		const price = (this.reserve / this.supply_no);
		const expect_payout = Math.floor(price * this.alice_no_amount);

		this.supply_no -= this.alice_no_amount;
